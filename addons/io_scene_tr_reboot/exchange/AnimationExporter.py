...
class AnimationExporter(AnimationExchanger):
    ...
    def export_animation(self, file_path: str, bl_armature_obj: bpy.types.Object) -> None:
        ...
        for bl_mesh_obj in Enumerable(bl_armature_obj.children).where(lambda o: isinstance(o.data, bpy.types.Mesh)):
            self.export_blend_shape_animation(tr_animation, bl_mesh_obj)

        resource_builder = ResourceBuilder(ResourceKey(ResourceType.ANIMATION, 0), CdcGame.SOTTR)
        tr_animation.write(resource_builder)
    ...
    def export_armature_animation(self, tr_animation: Animation, bl_armature_obj: bpy.types.Object) -> None:
        if self.apply_lara_bone_fix_constraints:
            self.apply_bone_fix_constraints(bl_armature_obj)
    ...
    def export_bone_animation(self, tr_animation: Animation, bl_bone: bpy.types.EditBone, global_bone_id: int, bl_attr_fcurves: dict[int, list[bpy.types.FCurve | None]]) -> None:
        ...
        for frame_idx in range(tr_animation.num_frames):
            ...
            for attr_idx, bl_elem_fcurves in bl_attr_fcurves.items():
                ...
                bone_frame.set_attr(attr_idx, cast(Sequence[float], attr_value))
    ...
    def collect_bone_fcurves(self, bl_armature_obj: bpy.types.Object) -> dict[int, dict[int, list[bpy.types.FCurve | None]]]:
        ...
        bl_action = BlenderHelper.get_action(bl_armature_obj)
    ...
    def export_blend_shape_animation(self, animation: Animation, bl_mesh_obj: bpy.types.Object) -> None:
        bl_blend_shape_fcurves = self.collect_blend_shape_fcurves(bl_mesh_obj)

        for global_blend_shape_id, bl_fcurve in bl_blend_shape_fcurves.items():
            if global_blend_shape_id in animation.blend_shape_tracks:
                continue
            ...
            animation.blend_shape_tracks[global_blend_shape_id] = blend_shape_frames

    def collect_blend_shape_fcurves(self, bl_mesh_obj: bpy.types.Object) -> dict[int, bpy.types.FCurve]:
        bl_blend_shape_fcurves: dict[int, bpy.types.FCurve] = {}
        bl_mesh = cast(bpy.types.Mesh, bl_mesh_obj.data)
        if bl_mesh.shape_keys is None:
            return bl_blend_shape_fcurves

        bl_action = BlenderHelper.get_action(bl_mesh.shape_keys)
        if bl_action is None:
            return bl_blend_shape_fcurves
        ...
        for bl_fcurve in bl_action.fcurves:
            ...
            bl_blend_shape_fcurves[global_blend_shape_id] = bl_fcurve

        return bl_blend_shape_fcurves

    def apply_bone_fix_constraints(self, bl_armature_obj: bpy.types.Object) -> None:
        (constraints, drm_name) = self.get_bone_fix_constraints()
        if bl_armature_obj.pose is None or len(constraints) == 0:
            return
        ...
        if len(missing_bone_ids) > 0:
            OperatorContext.log_warning(f"Bones with IDs {', '.join(missing_bone_ids)} not found - limbs may not be animated properly ingame. Please make sure you have {drm_name} imported.")
            return
    ...
    def bake_bone_constraints(self, bl_armature_obj: bpy.types.Object) -> None:
        ...
        with BlenderHelper.temporarily_show_all_bones(bl_armature_obj):
            for bl_pose_bone in bl_armature_obj.pose.bones:
                bl_pose_bone.select = len(bl_pose_bone.constraints) > 0
